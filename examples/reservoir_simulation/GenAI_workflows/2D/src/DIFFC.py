...
from torch.optim import Adam as Adam_clem
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def sample_clem(self, inn, batch_size=16, return_all_timesteps=False):
        image_size, channels = self.image_size, self.channels
        sample_fn = (
            self.p_sample_loop_clem
            if not self.is_ddim_sampling
            else self.ddim_sample_clem
        )
        return sample_fn(
            (batch_size, channels, image_size, image_size),
            inn,
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        # self.ds = Dataset(folder, self.image_size, augment_horizontal_flip =\
        #                   augment_horizontal_flip, convert_image_to = convert_image_to)
        dl = DataLoader(
            data_usee,
        ...
        with tqdm(
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    data = next(self.dl).to(device, dtype=torch.float32)
